...
<title>Virtualities - Editar</title>
...
<body>
	...
	<form:form modelAttribute="botModel" action="${contextPath}/bot/${botModel.id_bot}"
		method="put" class="container" style="margin: 15px auto;">
		<div class="row">
			<div class="col-sm-12">
				<div class="form-group">
					<label class="control-label" for="nome-label">Nome:</label>
					<form:input type="text" class="form-control" id="nome-label" path="name" />
						...
						<c:forEach items="${segmentos}" var="item" varStatus="status">
							<li class="list-group-item">
								<div class="form-check">
									<c:forEach items="${botModel.segment}" var="segmento">
										<c:if test="${segmento.name == item}">
									...
									<c:choose>
										<c:when test="${isChecked}">
											<form:checkbox path="listOfNames" class="form-check-input"
												value="${item}" id="${status.index}" checked="checked" />
											<label class="form-check-label" for="${status.index}">${item}</label>
											<c:set var="isChecked" value="false"/>
										</c:when>
										<c:when test="${not isChecked}">
											<form:checkbox path="listOfNames" class="form-check-input"
